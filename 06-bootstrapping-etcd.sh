...
# kubeadm-proj components are stateless and store cluster state in etcd. In this lab you will bootstrap a three node etcd cluster and configure it for high availability and secure remote access.
...
for instance in controller-0 controller-1 controller-2; do
  ...
  echo ssh -i kubeadm-proj.id_rsa ubuntu@$external_ip
  echo external_ip
done
...
ExecStart=/usr/local/bin/etcd \\
  --name ${ETCD_NAME} \\
  --cert-file=/etc/etcd/kubeadm-proj.pem \\
  --key-file=/etc/etcd/kubeadm-proj-key.pem \\
  --peer-cert-file=/etc/etcd/kubeadm-proj.pem \\
  --peer-key-file=/etc/etcd/kubeadm-proj-key.pem \\
...
sudo ETCDCTL_API=3 etcdctl member list \
  ...
  --cert=/etc/etcd/kubeadm-proj.pem \
  --key=/etc/etcd/kubeadm-proj-key.pem
# output
